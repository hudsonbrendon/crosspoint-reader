...
OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  LOG_DBG("OTA", "Checking for update (current: %s)", INKPOINT_VERSION);
...
bool OtaUpdater::isUpdateNewer() const {
  if (!updateAvailable || latestVersion.empty() || latestVersion == INKPOINT_VERSION) {
    return false;
  }
  ...
  const auto currentVersion = INKPOINT_VERSION;
