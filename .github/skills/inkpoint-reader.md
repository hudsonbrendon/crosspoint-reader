...
* .inkpoint/: SD-based binary cache for EPUB metadata and pre-rendered layout sections
...
```cpp
#define SETTINGS InkPointSettings::getInstance()  // User settings
#define APP_STATE InkPointState::getInstance()    // Runtime state
...
4. **Corrupt Cache Files**:
   - Delete `.inkpoint/` directory on SD card
...
9. 🔲 **Cache**: If EPUB modified, delete `.inkpoint/` and verify re-parse
...
**Location**: `.inkpoint/` directory on SD card root

**Structure**: `.inkpoint/epub_<hash>/{book.bin, progress.bin, cover.bmp, sections/*.bin}`
...
rm -rf /path/to/sd/.inkpoint/

# Delete specific book cache
rm -rf /path/to/sd/.inkpoint/epub_<hash>/

# Keep progress, delete only rendered sections
rm -rf /path/to/sd/.inkpoint/epub_<hash>/sections/
```
...
- After modifying:
  ...
  - Render settings in `InkPointSettings`
