...
INKPOINT_VERSION for the default (dev) environment.

Results in a version string like:  1.1.0-dev-feat-kosync-xpath-05c6cf8
Release environments are unaffected; they set INKPOINT_VERSION in the ini.
"""
...
def get_base_version(project_dir):
    ...
    if not config.has_option('inkpoint', 'version'):
        warn('No [inkpoint] version in platformio.ini; base version will be "0.0.0"')
        return '0.0.0'
    return config.get('inkpoint', 'version')
