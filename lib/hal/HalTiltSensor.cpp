...
void HalTiltSensor::update(const uint8_t mode, const uint8_t orientation, const bool inReader) {
  ...
  if ((mode != InkPointTiltPageTurn::TILT_OFF) && !_isAwake) {
    _isAwake = wake();
    return;
  } else if ((mode == InkPointTiltPageTurn::TILT_OFF) && _isAwake) {
    _isAwake = !deepSleep();
    return;
  }

  // If disabled, skip the rest of the polling logic and avoid unnecessary I2C traffic in non-reader activities
  if ((mode == InkPointTiltPageTurn::TILT_OFF) || !inReader) {
    return;
  }
  ...
  float tiltAxis;
  switch (orientation) {
    case InkPointOrientation::PORTRAIT:
      tiltAxis = mode == InkPointTiltPageTurn::TILT_INVERTED ? -gx : gx;
      break;
    case InkPointOrientation::INVERTED:
      tiltAxis = mode == InkPointTiltPageTurn::TILT_INVERTED ? gx : -gx;
      break;
    case InkPointOrientation::LANDSCAPE_CW:
      tiltAxis = mode == InkPointTiltPageTurn::TILT_INVERTED ? gy : -gy;
      break;
    case InkPointOrientation::LANDSCAPE_CCW:
      tiltAxis = mode == InkPointTiltPageTurn::TILT_INVERTED ? -gy : gy;
      break;
    default:
      tiltAxis = gx;
