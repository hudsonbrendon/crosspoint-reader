...
void* jpegOpen(const char* filename, int32_t* size) {
  // jpegClose() calls delete f — C API takes ownership; use nothrow so a failed
  // allocation returns nullptr rather than calling abort() (-fno-exceptions).
  HalFile* f = new (std::nothrow) HalFile();
  if (!f) {
    LOG_ERR("JPG", "OOM: HalFile in jpegOpen");
    return nullptr;
  }
